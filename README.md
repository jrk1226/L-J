...
Modelled on the systems bars and restaurants already run (Toast, SkyTab, Square, TouchBistro). Home screen on both sides: a list of every open check on the left (server colour bar, guest, minutes, amount, status pill; tabs for Open / Tabs / Mine / Closed) and the room as flat shapes on the right - the bar as a plain slab with numbered stools coloured by the bartender who owns them, or the restaurant's zones with tables, chairs drawn around them and a server-colour ring. Server chips on the tool row show who is working and how many they have open; tap yours to see only your checks. A top bar on every screen with the venue, the screen name, one search box, the clock and who is logged in; light by default with a Dark look; the check drawn like a receipt with a full-width Pay button; colour-coded category tabs, photo tiles, solid colour tiles for anything without a photo; required choices marked red until picked. Everything a finger has to hit is at least 44 pixels.
